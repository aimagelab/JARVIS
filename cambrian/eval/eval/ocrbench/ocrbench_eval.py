...
#     input_ids, pixel_valuess, image_sizes, prompts = zip(*batch)
#     input_ids = torch.stack(input_ids, dim=0)
#     pixel_valuess = torch.stack(pixel_valuess, dim=0)
#     return input_ids, pixel_valuess, image_sizes, prompts
...
def eval_model(args):
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, pixel_values, prompt = dataset[idx]
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                input_ids,
                # images=pixel_values.to(dtype=torch.float16, device='cuda', non_blocking=True),
